...
func startDisplayer() error {
	...
	//_, port, err := net.SplitHostPort(displayingListener.Addr().String())
	//if err != nil {
	//	fmt.Printf("cannot split host from (new) port %q: %v", displayingListener.Addr().String(), err)
	//}
	//config.SetChatListener(config.ChatListener.Ip, port)
