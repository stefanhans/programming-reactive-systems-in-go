...
func startPublisher() error {
	...
	publishingService := config.ChatServiceAddress()

	// Create publishingListener
	log.Printf("publishingListener: %q\n", publishingService)
	...
	displayText(fmt.Sprintf("<publishing service running: %s (%s)>", selfMember.Name, publishingService))
...
func handleSubscribeRequest(message *chatgroup.Message, addr net.Addr) error {
	...
	var m []*chatgroup.Member = cgMember
	message.MemberList = &chatgroup.MemberList{
		Member: m,
	}
