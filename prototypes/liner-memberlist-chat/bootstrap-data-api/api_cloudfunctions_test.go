...
func TestCfRefill(t *testing.T) {
	...
	dummy.BootstrapData.Config.MinRefillCandidates = 1
...
func TestCfConfig(t *testing.T) {
	...
	for n, tc := range testCases {
		...
		t.Run(n, func(t *testing.T) {
			...
			dummy.BootstrapData.Config.MinRefillCandidates = tc.minRefillCandidates
			...
			if !tc.expectedError {
				if bootstrapData.Config.MaxPeers != tc.maxPeers ||
					bootstrapData.Config.MinRefillCandidates != tc.minRefillCandidates {
					t.Errorf("expected MaxPeers/MinRefillCandidates %d/%d, received  %d/%d",
						tc.maxPeers, tc.minRefillCandidates,
						bootstrapData.Config.MaxPeers, bootstrapData.Config.MinRefillCandidates)
				}
			}
		})
	}
