...
// Todo correct naming for tests, testcommands, and testruns
...
func initTest(testName string) bool {
	...
	log.Printf("Received reply from init: %v\n", res.Status)
	...
	log.Printf("Received body from init: %v\n", string(body))
...
func resetTestRun() bool {

	// Send request to service
	res, err := http.Post(testSidecarUrl+"/resetrun",
		"application/x-www-form-urlencoded",
		strings.NewReader(""))
	if err != nil {
		displayError("failed to post tests request to test sidecar server", err)
